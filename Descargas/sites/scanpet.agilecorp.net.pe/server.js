...
app.use((req, res, next) => {
    ...
    let urlAnonymous = ['/logout', '/login', '/teacherForm', '/teachers/create', '/register', '/confirmaremail', '/recoverypassword'];
    if (req.originalUrl != '/') {
        ...
        if (urlFound == undefined) {
            if (req.isAuthenticated()) {
                ...
                res.locals.name = req.session.passport.user.name; // Aquí corregí 'req.session.req.session' a 'req.session.passport'

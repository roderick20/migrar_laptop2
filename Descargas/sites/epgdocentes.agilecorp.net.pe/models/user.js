...
console.log(user);
    ...
    this.select = async function (uniquieId) {
        ...
        ,[Created]
        ,[Modified]
        FROM [popl].[dbo].[User] WHERE [UniqueId] = '` + uniquieId + `' `;
    ...
    this.validarCuenta  = async function (codeAuth) {
        try {
            ...
            const query = `UPDATE [dbo].[Teacher] SET [Status] = 2 WHERE [codeAuth] =  @codeAuth`;
            const request = pool.request();
    ...
    this.recoverypassword  = async function (params) {
        try {
            ...
            const query = `UPDATE [dbo].[Teacher] SET [Password] = CONVERT(NVARCHAR(32), HashBytes('MD5', '${params.password}'), 2) WHERE [email] =  @email`;
            const request = pool.request();

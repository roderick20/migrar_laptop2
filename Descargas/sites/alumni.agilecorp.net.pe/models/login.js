...
login = async function (email, password) {
    ...
    try {
        pool = await sql.connect(config.db);
        const query  = `
             SELECT [Id], [UniqueId], [FirstName], [LastName], [Password], [LastAccess], [Status], [Email], 1 AS [Role]
            FROM [dbo].[Person] 
            WHERE [Email] = @Email AND [Password] = CONVERT(NVARCHAR(32), HashBytes('MD5', '${password}'), 2);
            
            SELECT [Id], [UniqueId], [Name], [LastName], [Password], [LastAccess], [Enabled], [Email], 2 AS [Role]
            FROM [dbo].[User] 
            WHERE [Email] = @Email AND [Password] = CONVERT(NVARCHAR(32), HashBytes('MD5', '${password}'), 2);`;
